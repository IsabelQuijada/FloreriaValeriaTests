...
const galeriaFunerariaImageManager = new GaleriaFunerariaImageManager();

// Generar productos para todas las categorías
const galeriaFunerariaProducts = galeriaFunerariaImageManager.getAllProducts();
...
const categoryNames = {
    ...
    'pie-caja-altar': 'Pie Caja/Altar'
};

class GaleriaFunerariaGallery {
    constructor() {
        ...
        window.galeriaFunerariaGallery = this;

        this.init();
    }

    init() {
        // Inicializar comportamientos específicos de la galería funeraria
        if (window.galeriaFunerariaUtils) {
            window.galeriaFunerariaUtils.initializeGaleriaFunerariaBehaviors();
        }
    ...
    setupProductManager() {
        ...
        // Crear ProductManager usando el método específico para galería funeraria
        this.productManager = ProductManager.createFuneralGallery('gallery-grid', galeriaFunerariaProducts);
        // Configurar la instancia actual de productos
        this.currentProducts = galeriaFunerariaProducts;
    ...
    setupFilters() {
        ...
        if (filterContainer) {
            const filters = [
                ...
                { category: 'coronas', label: 'Coronas' },
                { category: 'cruces', label: 'Cruces' },
                { category: 'cubre-caja', label: 'Cubre Caja' },
                { category: 'pie-caja-altar', label: 'Pie Caja/Altar' }
            ];
    ...
    applyFilter(category) {
        ...
        if (category === 'all') {
            filteredProducts = galeriaFunerariaProducts;
        } else {
            filteredProducts = galeriaFunerariaProducts.filter(product => product.category === category);
        }
    ...
    openModal(config) {
        const product = this.currentProducts.find(p => p.id === config.id) || 
                       galeriaFunerariaProducts.find(p => p.id === config.id);
        if (!product) return;
        this.currentProductIndex = this.currentProducts.indexOf(product);
        if (this.currentProductIndex === -1) {
            this.currentProducts = galeriaFunerariaProducts;
            this.currentProductIndex = galeriaFunerariaProducts.indexOf(product);
        }
    ...
    contactWhatsApp() {
        ...
        let message;
        if (window.galeriaFunerariaUtils) {
            message = window.galeriaFunerariaUtils.generateWhatsAppMessage(currentProduct.name, currentProduct.category);
        } else {
            message = `Hola, me interesa el arreglo funerario ${currentProduct.name}. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    contactFromCard(productId) {
        ...
        let message;
        if (window.galeriaFunerariaUtils) {
            message = window.galeriaFunerariaUtils.generateWhatsAppMessage(product.name, product.category);
        } else {
            message = `Hola, me interesa el arreglo funerario ${product.name}. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
        const whatsappUrl = `https://wa.me/523335558928?text=${encodeURIComponent(message)}`;
        window.open(whatsappUrl, '_blank', 'noopener,noreferrer');
    }
    ...
    hideLoading() {
        ...
        this.updateProductCounter(galeriaFunerariaProducts.length);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.galeriaFunerariaGallery = new GaleriaFunerariaGallery();
...
window.GaleriaFunerariaGallery = GaleriaFunerariaGallery;