// ===== GALERÍA DE CELEBRACIONES ESPECIALES CON COMPONENTES REUTILIZABLES =====

// Inicializar gestor de imágenes
const celebracionesEspecialesImageManager = new CelebracionesEspecialesImageManager();

// Generar productos para todas las categorías
const celebracionesEspecialesProducts = celebracionesEspecialesImageManager.getAllProducts();

// Mapeo de categorías para mostrar nombres amigables
const categoryNames = {
    'canastas-florales': 'Canastas Florales',
    'centros-de-mesa-festivos': 'Centros de Mesa Festivos',
    'detalles-en-forma-de-corazon': 'Detalles en Forma de Corazón'
};

class CelebracionesEspecialesGallery {
    constructor() {
        ...
        window.celebracionesGallery = this;

        this.init();
    }

    init() {
        // Inicializar comportamientos específicos de la galería de celebraciones especiales
        if (window.celebracionesEspecialesUtils) {
            window.celebracionesEspecialesUtils.initializeCelebracionesEspecialesBehaviors();
        }
    ...
    setupProductManager() {
        ...
        // Crear ProductManager usando el método específico para celebraciones especiales
        this.productManager = ProductManager.createCelebracionesGallery('gallery-grid', celebracionesEspecialesProducts);
        
        // Configurar la instancia actual de productos
        this.currentProducts = celebracionesEspecialesProducts;
    ...
    setupFilters() {
        ...
        if (filterContainer) {
            const filters = [
                ...
                { category: 'canastas-florales', label: 'Canastas Florales' },
                { category: 'centros-de-mesa-festivos', label: 'Centros de Mesa' },
                { category: 'detalles-en-forma-de-corazon', label: 'Detalles Corazón' }
            ];
    ...
    applyFilter(category) {
        ...
        if (category === 'all') {
            filteredProducts = celebracionesEspecialesProducts;
        } else {
            filteredProducts = celebracionesEspecialesProducts.filter(product => product.category === category);
        }
    ...
    openModal(config) {
        const product = this.currentProducts.find(p => p.id === config.id) || 
                       celebracionesEspecialesProducts.find(p => p.id === config.id);
        ...
        if (this.currentProductIndex === -1) {
            this.currentProducts = celebracionesEspecialesProducts;
            this.currentProductIndex = celebracionesEspecialesProducts.indexOf(product);
        }
    ...
    contactWhatsApp() {
        ...
        let message;
        if (window.celebracionesEspecialesUtils) {
            message = window.celebracionesEspecialesUtils.generateWhatsAppMessage(currentProduct.name, currentProduct.category);
        } else {
            message = `Hola, me interesa el ${currentProduct.name} para mi celebración especial. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    contactFromCard(productId) {
        const product = celebracionesEspecialesProducts.find(p => p.id === productId);
        if (!product) return;

        let message;
        if (window.celebracionesEspecialesUtils) {
            message = window.celebracionesEspecialesUtils.generateWhatsAppMessage(product.name, product.category);
        } else {
            message = `Hola, me interesa el ${product.name} para mi celebración especial. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    hideLoading() {
        ...
        this.updateProductCounter(celebracionesEspecialesProducts.length);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.celebracionesGallery = new CelebracionesEspecialesGallery();
...
window.CelebracionesEspecialesGallery = CelebracionesEspecialesGallery;