// ===== GALERÍA DE BODAS DE ENSUEÑO CON COMPONENTES REUTILIZABLES =====

// Inicializar gestor de imágenes
const bodasDeEnsuenioImageManager = new BodasDeEnsuenioImageManager();

// Generar productos para todas las categorías
const bodasDeEnsuenioProducts = bodasDeEnsuenioImageManager.getAllProducts();

// Mapeo de categorías para mostrar nombres amigables
const categoryNames = {
    'ramos-novia': 'Ramos de Novia',
    'arreglos-centros-mesa': 'Arreglos y Centros de Mesa',
    'templo': 'Decoración de Templo'
};

class BodasDeEnsuenioGallery {
    constructor() {
        ...
        window.bodasGallery = this;

        this.init();
    }

    init() {
        // Inicializar comportamientos específicos de la galería de bodas de ensueño
        if (window.bodasDeEnsuenioUtils) {
            window.bodasDeEnsuenioUtils.initializeBodasDeEnsuenioBehaviors();
        }
    ...
    setupProductManager() {
        ...
        // Crear ProductManager usando el método específico para bodas
        this.productManager = ProductManager.createBodasGallery('gallery-grid', bodasDeEnsuenioProducts);
        
        // Configurar la instancia actual de productos
        this.currentProducts = bodasDeEnsuenioProducts;
    ...
    setupFilters() {
        ...
        if (filterContainer) {
            const filters = [
                ...
                { category: 'ramos-novia', label: 'Ramos de Novia' },
                { category: 'arreglos-centros-mesa', label: 'Arreglos y Centros' },
                { category: 'templo', label: 'Decoración de Templo' }
            ];
    ...
    applyFilter(category) {
        ...
        if (category === 'all') {
            filteredProducts = bodasDeEnsuenioProducts;
        } else {
            filteredProducts = bodasDeEnsuenioProducts.filter(product => product.category === category);
        }
    ...
    openModal(config) {
        const product = this.currentProducts.find(p => p.id === config.id) || 
                       bodasDeEnsuenioProducts.find(p => p.id === config.id);
        ...
        if (this.currentProductIndex === -1) {
            this.currentProducts = bodasDeEnsuenioProducts;
            this.currentProductIndex = bodasDeEnsuenioProducts.indexOf(product);
        }
    ...
    contactWhatsApp() {
        ...
        let message;
        if (window.bodasDeEnsuenioUtils) {
            message = window.bodasDeEnsuenioUtils.generateWhatsAppMessage(currentProduct.name, currentProduct.category);
        } else {
            message = `Hola, me interesa el ${currentProduct.name} para mi boda. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    contactFromCard(productId) {
        const product = bodasDeEnsuenioProducts.find(p => p.id === productId);
        if (!product) return;

        let message;
        if (window.bodasDeEnsuenioUtils) {
            message = window.bodasDeEnsuenioUtils.generateWhatsAppMessage(product.name, product.category);
        } else {
            message = `Hola, me interesa el ${product.name} para mi boda. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    hideLoading() {
        ...
        this.updateProductCounter(bodasDeEnsuenioProducts.length);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.bodasGallery = new BodasDeEnsuenioGallery();
...
window.BodasDeEnsuenioGallery = BodasDeEnsuenioGallery;