// ===== GALERÍA DE RAMOS ELEGANTES CON COMPONENTES REUTILIZABLES =====

// Inicializar gestor de imágenes
const ramosElegantesImageManager = new RamosElegantesImageManager();

// Generar productos - todos están en una sola categoría
const ramosElegantesProducts = ramosElegantesImageManager.generateCategoryProducts('ramos-elegantes');

// Mapeo de categorías para mostrar nombres amigables
const categoryNames = {
    'ramos-elegantes': 'Ramos Elegantes'
};

class RamosElegantesGallery {
    constructor() {
        ...
        window.ramosElegantesGallery = this;

        this.init();
    }

    init() {
        // Inicializar comportamientos específicos de la galería de ramos elegantes
        if (window.ramosElegantesUtils) {
            window.ramosElegantesUtils.initializeRamosElegantessBehaviors();
        }
    ...
    setupProductManager() {
        ...
        // Crear ProductManager usando el método específico para ramos elegantes
        this.productManager = ProductManager.createRamosElegantesGallery('gallery-grid', ramosElegantesProducts);
        
        // Configurar la instancia actual de productos
        this.currentProducts = ramosElegantesProducts;
    ...
    setupFilters() {
        ...
        if (filterContainer) {
            const filters = [
                { category: 'all', label: 'Todos los Ramos Elegantes' }
            ];
    ...
    applyFilter(category) {
        ...
        if (category === 'all') {
            filteredProducts = ramosElegantesProducts;
        } else {
            filteredProducts = ramosElegantesProducts.filter(product => product.category === category);
        }
    ...
    openModal(config) {
        const product = this.currentProducts.find(p => p.id === config.id) || 
                       ramosElegantesProducts.find(p => p.id === config.id);
        ...
        if (this.currentProductIndex === -1) {
            this.currentProducts = ramosElegantesProducts;
            this.currentProductIndex = ramosElegantesProducts.indexOf(product);
        }
    ...
    showCurrentProduct() {
        ...
        if (modalCategory) {
            modalCategory.textContent = 'RAMOS-ELEGANTES';
        }
    ...
    contactWhatsApp() {
        ...
        let message;
        if (window.ramosElegantesUtils) {
            message = window.ramosElegantesUtils.generateWhatsAppMessage(currentProduct.name, currentProduct.category);
        } else {
    ...
    contactFromCard(productId) {
        const product = ramosElegantesProducts.find(p => p.id === productId);
        if (!product) return;

        let message;
        if (window.ramosElegantesUtils) {
            message = window.ramosElegantesUtils.generateWhatsAppMessage(product.name, product.category);
        } else {
    ...
    hideLoading() {
        ...
        this.updateProductCounter(ramosElegantesProducts.length);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.ramosElegantesGallery = new RamosElegantesGallery();
