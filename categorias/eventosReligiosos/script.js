// ===== GALERÍA DE EVENTOS RELIGIOSOS CON COMPONENTES REUTILIZABLES =====

// Inicializar gestor de imágenes
const eventosReligiososImageManager = new EventosReligiososImageManager();

// Generar productos para todas las categorías
const eventosReligiososProducts = eventosReligiososImageManager.getAllProducts();
...
class EventosReligiososGallery {
    constructor() {
        ...
        window.eventosReligiososGallery = this;

        this.init();
    }

    init() {
        // Inicializar comportamientos específicos de la galería de eventos religiosos
        if (window.eventosReligiososUtils) {
            window.eventosReligiososUtils.initializeEventosReligiososBehaviors();
        }
    ...
    setupProductManager() {
        ...
        // Crear ProductManager usando el método específico para eventos religiosos
        this.productManager = ProductManager.createEventosReligiososGallery('gallery-grid', eventosReligiososProducts);
        
        // Configurar la instancia actual de productos
        this.currentProducts = eventosReligiososProducts;
    ...
    setupFilters() {
        ...
        if (filterContainer) {
            const filters = [
                ...
                { category: 'arreglo-de-templo', label: 'Arreglo de Templo' },
                { category: 'bautizo', label: 'Bautizo' },
                { category: 'hermita', label: 'Hermita' }
            ];
    ...
    applyFilter(category) {
        ...
        if (category === 'all') {
            filteredProducts = eventosReligiososProducts;
        } else {
            filteredProducts = eventosReligiososProducts.filter(product => product.category === category);
        }
    ...
    openModal(config) {
        const product = this.currentProducts.find(p => p.id === config.id) || 
                       eventosReligiososProducts.find(p => p.id === config.id);
        ...
        if (this.currentProductIndex === -1) {
            this.currentProducts = eventosReligiososProducts;
            this.currentProductIndex = eventosReligiososProducts.indexOf(product);
        }
    ...
    contactWhatsApp() {
        ...
        let message;
        if (window.eventosReligiososUtils) {
            message = window.eventosReligiososUtils.generateWhatsAppMessage(currentProduct.name, currentProduct.category);
        } else {
            message = `Hola, me interesa el ${currentProduct.name} para mi evento religioso. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    contactFromCard(productId) {
        const product = eventosReligiososProducts.find(p => p.id === productId);
        if (!product) return;

        let message;
        if (window.eventosReligiososUtils) {
            message = window.eventosReligiososUtils.generateWhatsAppMessage(product.name, product.category);
        } else {
            message = `Hola, me interesa el ${product.name} para mi evento religioso. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    hideLoading() {
        ...
        this.updateProductCounter(eventosReligiososProducts.length);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.eventosReligiososGallery = new EventosReligiososGallery();
...
window.EventosReligiososGallery = EventosReligiososGallery;