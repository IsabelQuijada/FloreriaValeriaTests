// ===== GALERÍA DE CUMPLEAÑOS CON COMPONENTES REUTILIZABLES =====

// Inicializar gestor de imágenes
const cumpleanosImageManager = new CumpleanosImageManager();

// Generar productos para todas las categorías
const cumpleanosProducts = cumpleanosImageManager.getAllProducts();
...
class CumpleanosGallery {
    constructor() {
        ...
        window.cumpleanosGallery = this;

        this.init();
    }

    init() {
        // Inicializar comportamientos específicos de la galería de cumpleaños
        if (window.cumpleanosUtils) {
            window.cumpleanosUtils.initializeCumpleanosBehaviors();
        }
    ...
    setupProductManager() {
        ...
        // Crear ProductManager usando el método específico para cumpleaños
        this.productManager = ProductManager.createCumpleanosGallery('gallery-grid', cumpleanosProducts);
        
        // Configurar la instancia actual de productos
        this.currentProducts = cumpleanosProducts;
    ...
    applyFilter(category) {
        ...
        if (category === 'all') {
            filteredProducts = cumpleanosProducts;
        } else {
            filteredProducts = cumpleanosProducts.filter(product => product.category === category);
        }
    ...
    openModal(config) {
        const product = this.currentProducts.find(p => p.id === config.id) || 
                       cumpleanosProducts.find(p => p.id === config.id);
        ...
        if (this.currentProductIndex === -1) {
            this.currentProducts = cumpleanosProducts;
            this.currentProductIndex = cumpleanosProducts.indexOf(product);
        }
    ...
    contactFromCard(productId) {
        ...
        let message;
        if (window.cumpleanosUtils) {
            message = window.cumpleanosUtils.generateWhatsAppMessage(product.name, product.category);
        } else {
            message = `Hola, me interesa el ${product.name} para mi celebración de cumpleaños. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    hideLoading() {
        ...
        this.updateProductCounter(cumpleanosProducts.length);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.cumpleanosGallery = new CumpleanosGallery();
...
window.CumpleanosGallery = CumpleanosGallery;