// ===== GALERÍA DE RAMOS CLÁSICOS CON COMPONENTES REUTILIZABLES =====

// Inicializar gestor de imágenes
const ramosClasicosImageManager = new RamosClasicosImageManager();

// Generar productos para todas las categorías
const ramosClasicosProducts = ramosClasicosImageManager.getAllProducts();
...
class RamosClasicosGallery {
    constructor() {
        ...
        window.ramosClasicosGallery = this;

        this.init();
    }

    init() {
        // Inicializar comportamientos específicos de la galería de ramos clásicos
        if (window.ramosClasicosUtils) {
            window.ramosClasicosUtils.initializeRamosClasicoseBehaviors();
        }
    ...
    setupProductManager() {
        ...
        // Crear ProductManager usando el método específico para ramos clásicos
        this.productManager = ProductManager.createRamosClasicosGallery('gallery-grid', ramosClasicosProducts);
        
        // Configurar la instancia actual de productos
        this.currentProducts = ramosClasicosProducts;
    ...
    setupFilters() {
        ...
        if (filterContainer) {
            const filters = [
                ...
                { category: 'ramo-estilizado', label: 'Ramo Estilizado' },
                { category: 'ramo-girasoles', label: 'Ramo de Girasoles' },
                { category: 'ramo-mix', label: 'Ramo Mixto' },
                { category: 'ramo-rosas', label: 'Ramo de Rosas' },
                { category: 'ramo-tulipanes', label: 'Ramo de Tulipanes' }
            ];
    ...
    applyFilter(category) {
        ...
        if (category === 'all') {
            filteredProducts = ramosClasicosProducts;
        } else {
            filteredProducts = ramosClasicosProducts.filter(product => product.category === category);
        }
    ...
    openModal(config) {
        const product = this.currentProducts.find(p => p.id === config.id) || 
                       ramosClasicosProducts.find(p => p.id === config.id);
        ...
        if (this.currentProductIndex === -1) {
            this.currentProducts = ramosClasicosProducts;
            this.currentProductIndex = ramosClasicosProducts.indexOf(product);
        }
    ...
    contactWhatsApp() {
        ...
        let message;
        if (window.ramosClasicosUtils) {
            message = window.ramosClasicosUtils.generateWhatsAppMessage(currentProduct.name, currentProduct.category);
        } else {
            message = `Hola, me interesa el ${currentProduct.name}. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    contactFromCard(productId) {
        const product = ramosClasicosProducts.find(p => p.id === productId);
        if (!product) return;

        let message;
        if (window.ramosClasicosUtils) {
            message = window.ramosClasicosUtils.generateWhatsAppMessage(product.name, product.category);
        } else {
            message = `Hola, me interesa el ${product.name}. ¿Podrían darme más información sobre disponibilidad y precio?`;
        }
    ...
    hideLoading() {
        ...
        this.updateProductCounter(ramosClasicosProducts.length);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.ramosClasicosGallery = new RamosClasicosGallery();
...
window.RamosClasicosGallery = RamosClasicosGallery;